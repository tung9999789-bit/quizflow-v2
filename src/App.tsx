...
export default function App() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const formatted: Question[] = enhanced.map((q, i) => {
              ...
              <div className="space-y-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 px-8 py-4 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 hover:scale-[1.02] transition-all shadow-xl shadow-indigo-100"
                  >
                    <Plus size={20} />
                    Tạo bài tập mới
                  </button>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 bg-slate-50 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all font-bold"
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <button 
                    onClick={startQuiz}
                    disabled={!quizCode || !userName}
                    className="w-full py-4 bg-indigo-600 text-white font-bold rounded-2xl shadow-lg shadow-indigo-100 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 group transition-all"
                  >
                    Bắt đầu làm bài
                ...
                <div className="max-w-xl mx-auto">
                  ...
                  {loading && (
                    <motion.div 
                      initial={{ opacity: 0 }} 
                      animate={{ opacity: 1 }} 
                      className="mt-8 p-6 bg-indigo-50 rounded-2xl flex items-center gap-4 border border-indigo-100"
                    >
                ...
                <div className="space-y-8">
                  <div className="flex flex-col md:flex-row gap-6 items-end justify-between bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <div className="flex-1 space-y-1 w-full">
                      ...
                      <input 
                        ...
                        className="w-full text-xl font-bold text-slate-800 border-none focus:ring-0 outline-none p-0 transition-all"
                      />
                    </div>
                    <div className="flex items-center gap-3 w-full md:w-auto">
                      <button 
                        onClick={() => exportQuizToExcel(newQuizTitle, newQuizQuestions)}
                        className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 text-white font-bold rounded-2xl shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all"
                      >
                        <Download size={18} />
                        Xuất Excel
                      </button>
                      <button 
                        ...
                        className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-indigo-600 text-white font-bold rounded-2xl shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all"
                      >
                        <CheckCircle size={18} />
                        Lưu bài
                      </button>
                      ...
                      <div key={q.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex items-start gap-4">
                            <span className="flex-shrink-0 text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded">
                              CÂU {String(idx + 1).padStart(2, '0')}
                            </span>
                            <p className="text-lg font-bold text-slate-800 pt-1">{q.question}</p>
                          </div>
                          <button 
                            onClick={() => setNewQuizQuestions(p => p.filter(item => item.id !== q.id))}
                        ...
                        <div className="mt-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                          <div className="flex items-center gap-2 mb-2">
                             <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Giải thích</span>
                          </div>
                  ...
                  <div className="flex justify-center pt-8">
                    <button 
                      ...
                      className="px-8 py-3 text-gray-500 font-bold hover:text-gray-900 transition-colors"
                    >
                      Hủy và quay lại
                    </button>
                  </div>
          ...
          {view === 'taking' && currentQuiz && (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-xl font-bold">
              ...
              <AnimatePresence mode="wait">
                <motion.div 
                  key={currentQuiz.questions[currentQuestionIndex].id}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="bg-white p-8 md:p-12 rounded-[32px] border border-slate-100 shadow-sm space-y-8"
                >
                  ...
                  <div className="grid gap-3">
                    {currentQuiz.questions[currentQuestionIndex].options.map((option, oIdx) => (
                      <button 
                        ...
                        className={`w-full p-5 text-left rounded-2xl border-2 transition-all font-semibold flex items-center justify-between group ${
                          userAnswers[currentQuiz.questions[currentQuestionIndex].id] === option 
              ...
              <div className="flex items-center justify-between gap-4">
                <button 
                  ...
                  className="px-8 py-4 bg-white text-gray-400 font-bold rounded-2xl border border-gray-100 hover:text-gray-900 hover:border-gray-300 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                >
                  Quay lại
                </button>
                {currentQuestionIndex === currentQuiz.questions.length - 1 ? (
                  <button 
                    ...
                    className="flex-1 py-4 bg-indigo-600 text-white font-bold rounded-2xl shadow-xl shadow-indigo-100 hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                  >
                    Nộp bài
                    <CheckCircle size={20} />
                  </button>
                  ...
                  <button 
                    onClick={() => setCurrentQuestionIndex(p => p + 1)}
                    disabled={!userAnswers[currentQuiz.questions[currentQuestionIndex].id]}
                    className="flex-1 py-4 bg-indigo-600 text-white font-bold rounded-2xl shadow-xl shadow-indigo-100 hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                  >
                    Câu tiếp theo
                    <ArrowRight size={20} />
            ...
            >
              <div className="text-center space-y-6">
                <div className="relative inline-block">
                  <div className="w-32 h-32 bg-indigo-600 rounded-[40px] rotate-6 flex items-center justify-center text-white shadow-2xl shadow-indigo-200">
                    <Trophy size={60} className="-rotate-6" />
                  </div>
                  <div className="absolute -top-4 -right-4 w-12 h-12 bg-yellow-400 text-white rounded-2xl flex items-center justify-center font-black animate-pulse">
                    {Math.round((attemptResult.score / attemptResult.total) * 100)}%
                  </div>
                </div>
                <div className="space-y-2">
                  <h2 className="text-4xl font-black">{attemptResult.score === attemptResult.total ? 'Tuyệt vời!' : 'Hoàn thành bài viết!'}</h2>
                  <p className="text-gray-500 font-medium">Chúc mừng <span className="font-bold text-gray-900">{attemptResult.userName}</span> đã hoàn thành bài tập.</p>
                </div>
                
                <div className="flex items-center justify-center gap-8 py-8 border-y border-gray-100">
                  <div className="text-center">
                    <p className="text-3xl font-black text-indigo-600">{attemptResult.score}</p>
                    <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Câu đúng</p>
                  </div>
                  <div className="w-px h-8 bg-gray-100" />
                  <div className="text-center">
                    <p className="text-3xl font-black text-gray-300">{attemptResult.total}</p>
                    <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Tổng câu</p>
                  </div>
                </div>
              </div>

              <div className="space-y-6">
                <h3 className="text-xl font-bold px-2">Xem lại đáp án</h3>
                <div className="grid gap-4">
                  {currentQuiz.questions.map((q, idx) => {
                    const isCorrect = attemptResult.answers[q.id] === q.correctAnswer;
                    return (
                      <div key={q.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                        <div className="flex items-start gap-4">
                          <span className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${isCorrect ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
                            {idx + 1}
                          </span>
                          <p className="font-bold text-slate-800 pt-1">{q.question}</p>
                        </div>
                        <div className="grid gap-2">
                          <div className={`p-4 rounded-xl flex items-center justify-between ${isCorrect ? 'bg-emerald-50 border border-emerald-100' : 'bg-red-50 border border-red-100'}`}>
                            <div className="flex items-center gap-3">
                              {isCorrect ? <CheckCircle size={18} className="text-emerald-600" /> : <XCircle size={18} className="text-red-600" />}
                              <span className={`text-sm font-bold ${isCorrect ? 'text-emerald-800' : 'text-red-800'}`}>
                                Lựa chọn của bạn: {attemptResult.answers[q.id]}
                              </span>
                            </div>
                          </div>
                          {!isCorrect && (
                            <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                              <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest block mb-1">Đáp án đúng</span>
                              <p className="text-sm font-bold text-emerald-800">{q.correctAnswer}</p>
                            </div>
                          )}
                          <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                             <p className="text-xs text-slate-600 leading-relaxed italic"> <span className="font-bold text-slate-400 not-italic uppercase tracking-widest text-[9px] mr-2">Giải thích:</span> {q.explanation}</p>
                          </div>
                        </div>
                      </div>
                    );
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-8">
                <button 
                  ...
                  className="flex-1 py-4 bg-gray-900 text-white font-bold rounded-2xl hover:bg-black transition-all flex items-center justify-center gap-2"
                >
                  <RefreshCw size={20} />
                  Làm lại
                </button>
                <button 
                  ...
                  className="flex-1 py-4 bg-white text-gray-900 font-bold rounded-2xl border-2 border-gray-100 hover:border-indigo-400 transition-all flex items-center justify-center gap-2"
                >
                  <Home size={20} />
                  Quay về trang chủ
                </button>
